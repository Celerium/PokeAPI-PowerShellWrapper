<#
    .SYNOPSIS
        Gets a BCDR agent report from the Poke API.

    .DESCRIPTION
        The Get-PokeAgentReport script gets a BCDR agent report from the Poke API.

        Can be run against a single BCDR device or all BCDR devices registered in the Poke portal

        This is a proof of concept script. It is not intended to be used in production.

    .PARAMETER Api_Key_Public
        Defines your Poke API public key.

    .PARAMETER Api_Key_Secret
        Defines your Poke API secret key.

    .PARAMETER APIEndPoint
        Define what Poke endpoint to connect to.

        The default is https://pokeapi.co/api/v2
    ...
    .EXAMPLE
        Get-PokeAgentReport -Api_Key_Public 12345 -Api_Key_Secret 12345

        Gets all BCDR agents from every registered BCDR in the Poke portal.

        Returned data is outputted to a CSV file.

    .EXAMPLE
        Get-PokeAgentReport -Api_Key_Public 12345 -Api_Key_Secret 12345 -serialNumber 1234567890
    ...
    .EXAMPLE
        Get-PokeAgentReport -Api_Key_Public 12345 -Api_Key_Secret 12345 -Report All
...
#Region    [ Prerequisites ]
    ...
    $ScriptName = 'Get-PokeAgentReport'
    ...
    Try {
        ...
        If(Get-Module -ListAvailable -Name PokeAPI) {
            Import-module PokeAPI -ErrorAction Stop
        }
        Else {
            Install-Module PokeAPI -Force -ErrorAction Stop
            Import-Module PokeAPI -ErrorAction Stop
        }
    }
    Catch {
    ...
    #Settings Poke login information
    Add-PokeBaseURI -base_uri $APIEndpoint
    Add-PokeAPIKey -Api_Key_Public $Api_Key_Public -Api_Key_Secret $Api_Key_Secret -ErrorAction Stop
...
Write-Output " - (1/3) - $(Get-Date -Format MM-dd-HH:mm) - Getting Poke agents from the API"
...
#Region     [ Poke API ]
    ...
    Try{
        ...
        if ($serialNumber){
            ...
            $PokeBCDR = Get-PokeDevice -serialNumber $serialNumber
            $PokeAgents = (Get-PokeAgent -serialNumber $serialNumber).items

            $PokeAgentResults = $PokeAgents | Select-Object `
                @{Name='resellerCompanyName';Expression={$PokeBCDR.resellerCompanyName}}, `
                @{Name='clientCompanyName';Expression={$PokeBCDR.clientCompanyName}}, `
                @{Name='BCDR';Expression={$PokeBCDR.Name}}, `
                @{Name='BCDRip';Expression={$PokeBCDR.internalIP}}, `
                @{Name='Model';Expression={$PokeBCDR.Model}}, `
                @{Name='serialNumber';Expression={$PokeBCDR.serialNumber}}, `
            ...
            $PokeBCDR = (Get-PokeDevice).items
            $PokeAgentResults = ForEach ($BCDR in $PokeBCDR){

                $Agents = (Get-PokeAgent -serialNumber $BCDR.serialNumber).items
...
#Region     [ CSV Report ]
    Try{
        If($Report -eq 'All' -or $Report -eq 'CSV'){
            ...
            $PokeAgentResults | Sort-Object clientCompanyName | Select-Object $ScriptName,* | Export-Csv $CSVReport -NoTypeInformation
        }
    }
    Catch{
        Write-Host $_.Exception.Message -ForegroundColor Red -BackgroundColor Black
...
#Region    [ HTML Report]
    ...
    Try{
        If ($Report -eq 'All' -or $Report -eq 'HTML'){
            ...
            $TotalAgents                = ($PokeBCDR.agentCount | Measure-Object -Sum).sum
            $AgentVersion               = ($PokeAgentResults.AgentVersion | Select-Object -Unique).Count
            $TotalProtectedVolumes      = ($PokeAgentResults.protectedVolumesCount | Measure-Object -Sum).sum
            $TotalUnProtectedVolumes    = ($PokeAgentResults.unprotectedVolumesCount | Measure-Object -Sum).sum
            $TotalPaused                = ($PokeAgentResults | Where-Object {$_.isPaused -eq 'True'}).count
            $TotalArchived              = ($PokeAgentResults | Where-Object {$_.isArchived -eq 'True'}).count
    ...
    #Region    [ Example HTML Report Data\Structure ]
        ...
        $TableResults = $PokeAgentResults | ConvertTo-Html -As Table -Fragment -Property clientCompanyName,Agent,AgentVersion,protectedVolumeNames,unprotectedVolumeNames,isPaused,isArchived `
                                        -PostContent    '   <ul>
