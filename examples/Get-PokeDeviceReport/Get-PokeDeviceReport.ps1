<#
    .SYNOPSIS
        Gets an BCDR device report from the Poke API.

    .DESCRIPTION
        The Get-PokeDeviceReport script gets an BCDR device report from the Poke API.
    ...
    .EXAMPLE
        Get-PokeDeviceReport -Api_Key_Public 12345 -Api_Key_Secret 12345

        Gets all registered BCDR's from the Poke portal.

        Returned data is outputted to a CSV file.

    .EXAMPLE
        Get-PokeDeviceReport -Api_Key_Public 12345 -Api_Key_Secret 12345 -serialNumber 1234567890

        Gets the defined BCDR device from the Poke portal.

        Returned data is outputted to a CSV file.

    .EXAMPLE
        Get-PokeDeviceReport -Api_Key_Public 12345 -Api_Key_Secret 12345 -Report All
...
#Region    [ Prerequisites ]
    ...
    $ScriptName = 'Get-PokeDeviceReport'
...
Write-Output " - (1/3) - $(Get-Date -Format MM-dd-HH:mm) - Getting Poke BCDR's from the API"
...
#Region     [ Poke API ]
    ...
    Try{
        ...
        if ($serialNumber){
            $PokeDeviceResults = Get-PokeDevice -serialNumber $serialNumber
        }
        else{
            $PokeDeviceResults = Get-PokeDevice
            $PokeDeviceResults = ($PokeDeviceResults).items
        }

        $PokeAgentResults = $PokeDeviceResults | Select-Object `
...
#Region    [ HTML Report]
    ...
    Try{
        If ($Report -eq 'All' -or $Report -eq 'HTML'){
            ...
            $TotalDevices   = ($PokeAgentResults.serialNumber).count
            $DeviceTypes    = ($PokeAgentResults.Model | Select-Object -Unique).count
            $LocalStorage   = $(Convert-BytesToSize -Size $( ($PokeDeviceResults.localStorageUsed | Measure-Object -Property Size -Sum).sum * 1024))
            $OffsiteStorage = $(Convert-BytesToSize -Size $( ($PokeDeviceResults.offsiteStorageUsed | Measure-Object -Property Size -Sum).sum ))
            $ActiveTickets  = ($PokeAgentResults.activeTickets | Measure-Object -Sum).sum
            $TotalAgents    = ($PokeAgentResults.agentCount | Measure-Object -Sum).sum
            $TotalShares    = ($PokeAgentResults.shareCount | Measure-Object -Sum).sum
            $TotalAlerts    = ($PokeAgentResults.alertCount | Measure-Object -Sum).sum
    ...
    #Region    [ Example HTML Report Data\Structure ]
        ...
        $TableResults = $PokeAgentResults | ConvertTo-Html -As Table -Fragment -Property clientCompanyName,status,Name,Model,localStorageUsed,offsiteStorageUsed,lastSeenDate,warrantyExpire `
                                        -PostContent    '   <ul>
